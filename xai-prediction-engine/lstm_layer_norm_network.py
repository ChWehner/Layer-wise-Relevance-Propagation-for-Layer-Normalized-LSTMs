...
# This class implements LRP for a layer normalized LSTM 
# followed by two dense layers and a softmax activation function.
class LSTM_Layer_Norm_Network:

    def __init__(self, n_hidden_lstm, n_hidden_dense_one, n_hidden_dense_two, embedding_dim, n_classes, weights, mode="None",
                 debug=False):
    ...
    def full_pass(self, x):
        '''
        :param x:       batch of embeddings. (input vector of the LSTM)
    ...
    def lrp_linear_layer(self, h_in, w, b, h_out, R_out, bias_nb_units, eps=0., bias_factor=1.0):
        '''
    ...
    def lrp_norm_layer(self, x, beta, R_out, x_dash, x_dash_dash, mu):
        '''
        ...
        Returns:
        - R_in:           relevance at layer input, of shape (batch_size, D)
        - R_sink:         total relevance that is absorbed in this layer
        '''

        stabilizer = tf.constant(1e-300, dtype=tf.float64)  # so no true division by 0 happens

        R_x_dash = x_dash_dash * (R_out / (x_dash_dash + beta + stabilizer))  # R_x_dash==R_x_dash_dash
        R_b = beta * (R_out / (x_dash_dash + beta + stabilizer))

        R_in = x * (R_x_dash / (x_dash + stabilizer))
        ...
        R_sink = tf.reduce_sum(R_b, axis=1)  # o check for relevance conservation 
    ...
    def lrp(self, x, y=None, eps=1e-3, bias_factor=0.0):
        ...
        # relevance at the pre-softmax output of the model
        R_T = output_dense_two * R_out_mask

        # relevance of the second dense layer
        ...
        # relevance of the first dense layer
        ...
        # compute relevance of the layer normalized LSTM Layer
        ...
        # remaining relevance is what is absored in the normalization layers. Use this for relevance conservation check.
        rest = lrp_pass[-1]
    ...
    def lrp_lstm(self, x, output_lstm, R_T, batch_size, eps=1e-3, bias_factor=0.0):
        ...
        T = x.shape[1]
        # get all the varibales needed to trace back the relevance
        ...
        # make all the helper variables ready to update the relevance values
        ...
        def update(input_tuple, t):
            ...
            Rc_norm_t, R_c_sink = self.lrp_norm_layer(x=c[t, :], beta=self.beta_state_norm,
                                            ...
                                            x_dash_dash=c_norm_x_dash_dash[t, :], mu=c_mean[t, :])
            if self.debug:
            ...
            # distribution of R_g for the x- and h-gate
            ...
            Rx_norm_t, R_x_sink = self.lrp_norm_layer(x=gate_x[t, :, self.idx_c],
                                            beta=self.beta_kernel_norm[self.idx_c] + self.b_lstm[self.idx_c],
            ...
            Rx_t = self.lrp_linear_layer(h_in=x[:, t], w=self.W_x[:, self.idx_c], b=zero_bias[self.idx_c],
                                         ...
                                         eps=eps, bias_factor=bias_factor)

            # only use if absorbed relevance in normalization layers bring to much instability
            if self.mode == "norm":
                Rx_t = self.normalise(Rx_t)

            if self.debug:
                tf.print('Rx_t: Output relevance', tf.reduce_sum(Rx_t, axis=1))

            Rh_norm_t, R_h_sink = self.lrp_norm_layer(x=gate_h[t, :, self.idx_c],
                                            beta=self.beta_recurrent_norm[self.idx_c] + self.b_lstm[self.idx_c],
            ...
            # sum up the total lost relevance in all normalization layers
            ...
            # merge the relevance of the recurrent and cell state together 
            # for the next pass through the lstm layer     
            if t != 0:
                Rc_t += Rh_t
    ...
    def normalise(self, Rx_t):
        '''
        :param Rx_t: [batch, elems]
        :return: normalized relevance values 
        '''
        mean_i = tf.reduce_mean(Rx_t, axis=-1, keepdims=True)
        var_i = tf.math.reduce_variance(Rx_t, axis=-1, keepdims=True)
        Rx_t_dash = (Rx_t - mean_i)
        Rx_t_norm = Rx_t_dash / tf.math.sqrt(var_i)
